...
def viz_similarities(
    ...
    for t in tqdm(range(num_frames)):
        ...
        print(f"t_ix: {t_ix}")
        print(f"Shape of sim array: {sim.shape}")
        sim_t = max(sim[t_ix]) if len(sim) > 0 else 0
...
def load_model(checkpoint_path: Path, arch: str) -> torch.nn.Module:
    ...
    if arch == "i3d_mlp":
        ...
        model = InceptionI3d(num_classes=1064, num_in_frames=16, include_embds=True)
    else:
        raise ValueError(f"Unrecognized architecture {arch}")
    ...
    # Adjust keys if they were saved with DataParallel
